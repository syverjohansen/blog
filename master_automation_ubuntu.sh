...
run_scrapers_for_sport() {
    ...
    if [[ -f "race_scrape.py" ]]; then
        ...
        log_message "  ========== RACE_SCRAPE $sport OUTPUT BEGIN =========="
        if python3 race_scrape.py 2>&1 | tee -a "$LOG_FILE"; then
            log_message "  ========== RACE_SCRAPE $sport OUTPUT END =========="
    ...
    if [[ -f "weekend_scrape.py" ]]; then
        ...
        log_message "  ========== WEEKEND_SCRAPE $sport OUTPUT BEGIN =========="
        if python3 weekend_scrape.py 2>&1 | tee -a "$LOG_FILE"; then
            log_message "  ========== WEEKEND_SCRAPE $sport OUTPUT END =========="
