...
# Ski Statistics Update Script for daehl-e.com
...
# Use a more portable method that works on different systems
if date -v-1d >/dev/null 2>&1; then
    ...
    # Fallback: calculate yesterday using seconds
    YESTERDAY_EPOCH=$(($(date +%s) - 86400))
    YESTERDAY=$(date -d "@$YESTERDAY_EPOCH" +"%m/%d/%Y" 2>/dev/null || date -r $YESTERDAY_EPOCH +"%m/%d/%Y")
fi
