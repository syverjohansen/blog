...
def process_head2head_data(df, gender):
    """Process chronological data for head-to-head comparisons"""
    ...
    required_columns = ['ID', 'Skier', 'Season', 'Race', 'Date', 'City', 'Country', 
                       'Distance', 'Technique', 'MS', 'Place', 'Elo']
    ...
    df_filtered['MS'] = df_filtered['MS'].fillna('0').astype(str)
    ...
    df_filtered['Technique'] = df_filtered['Technique'].fillna('Unknown')
    ...
    for skier_id, skier_group in df_filtered.groupby('ID'):
        ...
        for _, race in skier_group.iterrows():
            race_data = {
                ...
                'distance': clean_value(race['Distance']),
                'technique': clean_value(race['Technique']),
                'ms': clean_value(race['MS']),
...
def create_race_lookup(df, gender):
    """Create a lookup table for race information"""
    df_filtered = df[df['Race'] != 0].copy()
    ...
    # Get unique races
    races = df_filtered[['Season', 'Race', 'Date', 'City', 'Country', 'Distance', 'Technique']].drop_duplicates()
...
men_data = process_head2head_data(men_df, 'M')
men_races = create_race_lookup(men_df, 'M')

ladies_data = process_head2head_data(ladies_df, 'L')
ladies_races = create_race_lookup(ladies_df, 'L')

# Create output directory if it doesn't exist
output_dir = '/Users/syverjohansen/blog/daehl-e/static/python/cross-country/excel365'
...
print("Head-to-head data processing complete!")
