...
def process_all_time_records(df, file_path):
    ...
    # Define column mapping to simpler names for biathlon
    column_mapping = {
        'Elo': 'Overall',
        'Sprint_Elo': 'Sprint',
        'Pursuit_Elo': 'Pursuit',
    ...
    # Reorder columns for biathlon
    all_time_records = all_time_records[[
        "Place", "Skier", "Nation", "Overall", "Sprint", 
        "Pursuit", "Individual", "Mass Start", "ID"
    ]]
    ...
    # Print debug for a top biathlete (you can change this name)
    print(f"\nTop biathlete's record from {file_path}:")
...
for i, df in enumerate(chronos):
    ...
    file_path = f"/Users/syverjohansen/blog/daehl-e/static/python/biathlon/excel365/{chronos_str[i]}_all_time.json"
