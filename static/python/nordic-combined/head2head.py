...
# Read the CSV files for Nordic Combined
men_df = pd.read_csv('/Users/syverjohansen/ski/elo/python/nordic-combined/polars/excel365/men_chrono.csv')
ladies_df = pd.read_csv('/Users/syverjohansen/ski/elo/python/nordic-combined/polars/excel365/ladies_chrono.csv')
...
def process_nordic_combined_head2head_data(df, gender):
    """Process chronological data for nordic combined head-to-head comparisons"""
    
    print(f"Processing Nordic Combined {gender} data...")
    ...
    for skier_id, skier_group in df_filtered.groupby('ID'):
        ...
        for _, race in skier_group.iterrows():
            race_data = {
                ...
                'event': clean_value(race['RaceType']),  # In nordic combined, RaceType column contains events
...
def create_nordic_combined_race_lookup(df, gender):
    """Create a lookup table for nordic combined race information"""
    df_filtered = df[df['Race'] != 0].copy()
    ...
    for _, race in races.iterrows():
        ...
        race_lookup[key] = {
            ...
            'city': clean_value(race['City']),
            'country': clean_value(race['Country']),
            'event': clean_value(race['RaceType'])  # Event instead of distance for nordic combined
        }
    
    return race_lookup

# Process both datasets
print("Starting Nordic Combined data processing...")

men_data = process_nordic_combined_head2head_data(men_df, 'M')
men_races = create_nordic_combined_race_lookup(men_df, 'M')

ladies_data = process_nordic_combined_head2head_data(ladies_df, 'L')
ladies_races = create_nordic_combined_race_lookup(ladies_df, 'L')

# Create output directory if it doesn't exist
output_dir = '/Users/syverjohansen/blog/daehl-e/static/python/nordic-combined/excel365'
...
print("Nordic Combined head-to-head data processing complete!")
