...
# Read the CSV files for Alpine
men_df = pd.read_csv('/Users/syverjohansen/ski/elo/python/alpine/polars/excel365/men_chrono.csv')
ladies_df = pd.read_csv('/Users/syverjohansen/ski/elo/python/alpine/polars/excel365/ladies_chrono.csv')
...
def process_alpine_head2head_data(df, gender):
    """Process chronological data for alpine head-to-head comparisons"""
    
    print(f"Processing Alpine {gender} data...")
    ...
    df_filtered['Distance'] = df_filtered['Distance'].fillna('Unknown')
    ...
    for skier_id, skier_group in df_filtered.groupby('ID'):
        ...
        for _, race in skier_group.iterrows():
            race_data = {
                ...
                'event': clean_value(race['Distance']),  # In alpine, Distance column contains events
...
def create_alpine_race_lookup(df, gender):
    """Create a lookup table for alpine race information"""
    df_filtered = df[df['Race'] != 0].copy()
    ...
    # Get unique races
    races = df_filtered[['Season', 'Race', 'Date', 'City', 'Country', 'Distance']].drop_duplicates()
    ...
    for _, race in races.iterrows():
        ...
        race_lookup[key] = {
            ...
            'city': clean_value(race['City']),
            'country': clean_value(race['Country']),
            'event': clean_value(race['Distance'])  # Event instead of distance for alpine
        }
    
    return race_lookup

# Process both datasets
print("Starting Alpine data processing...")

men_data = process_alpine_head2head_data(men_df, 'M')
men_races = create_alpine_race_lookup(men_df, 'M')

ladies_data = process_alpine_head2head_data(ladies_df, 'L')
ladies_races = create_alpine_race_lookup(ladies_df, 'L')

# Create output directory if it doesn't exist
output_dir = '/Users/syverjohansen/blog/daehl-e/static/python/alpine/excel365'
...
print("Alpine head-to-head data processing complete!")
