...
# Read the CSV files for Ski Jump
men_df = pd.read_csv('/Users/syverjohansen/ski/elo/python/skijump/polars/excel365/men_chrono.csv')
ladies_df = pd.read_csv('/Users/syverjohansen/ski/elo/python/skijump/polars/excel365/ladies_chrono.csv')
...
def process_skijump_head2head_data(df, gender):
    """Process chronological data for ski jump head-to-head comparisons"""
    
    print(f"Processing Ski Jump {gender} data...")
    ...
    for skier_id, skier_group in df_filtered.groupby('ID'):
        ...
        for _, race in skier_group.iterrows():
            race_data = {
                ...
                'event': clean_value(race['RaceType']),  # In ski jump, RaceType column contains hill sizes
...
def create_skijump_race_lookup(df, gender):
    """Create a lookup table for ski jump race information"""
    df_filtered = df[df['Race'] != 0].copy()
    ...
    for _, race in races.iterrows():
        ...
        race_lookup[key] = {
            ...
            'city': clean_value(race['City']),
            'country': clean_value(race['Country']),
            'event': clean_value(race['RaceType'])  # Hill size instead of distance for ski jump
        }
    
    return race_lookup

# Process both datasets
print("Starting Ski Jump data processing...")

men_data = process_skijump_head2head_data(men_df, 'M')
men_races = create_skijump_race_lookup(men_df, 'M')

ladies_data = process_skijump_head2head_data(ladies_df, 'L')
...
print("Ski Jump head-to-head data processing complete!")
