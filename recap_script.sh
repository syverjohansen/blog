...
# Set timezone to GMT/UTC for all operations
export TZ=UTC

# Get date calculations in UTC (macOS BSD date compatible)
...
SEVEN_DAYS_AGO_UTC=$(date -u -v-7d '+%Y-%m-%d')
SEVEN_DAYS_AGO_MMDDYYYY=$(date -u -v-7d '+%m/%d/%Y')
...
check_races_last_7_days() {
    ...
    # Create array of dates from 7 days ago to yesterday (inclusive) - macOS BSD date compatible
    local check_dates=()
    for i in {7..1}; do
        local check_date=$(date -u -v-${i}d '+%m/%d/%Y')
...
process_sport_recap() {
    ...
    if [[ -f "$standings_script" ]]; then
        ...
        cd "$polars_dir" && python standings_scrape.py >/dev/null 2>&1
    ...
    if [[ -f "$recap_script" ]]; then
        ...
        cd "$(dirname "$recap_script")" && Rscript race-recap2.R >/dev/null 2>&1
        ...
        for gender in "${genders[@]}"; do
            for file_type in "${file_types[@]}"; do
                ...
                if [[ -f "$excel_file" ]]; then
                    ...
                    python "$BLOG_DIR/static/python/excel_to_hugo_multiple_sheets.py" "$excel_file" "$recap_output_dir" >/dev/null 2>&1
                    if [[ $? -eq 0 ]]; then
